...
void acquisition::Capture::load_cameras()
{
    ...
    for (int i = 0; i < numCameras_; i++)
    {
        ...
        ROS_INFO_STREAM("  -" << cam.get_id() << " " << cam.getTLNodeStringValue("DeviceModelName") << " "
        ...
        for (int i = 0; i < numCameras_; i++)
        {
            ...
            if (cam.get_id().compare(cam_ids_[j]) == 0)
            {
                ...
                if (PUBLISH_CAM_INFO_)
                {
                    ...
                    {
                        ci_msg->P = {proj_coeff_vec_[j][0], proj_coeff_vec_[j][1],  proj_coeff_vec_[j][2],
                                     proj_coeff_vec_[j][3], proj_coeff_vec_[j][4],  proj_coeff_vec_[j][5],
                                     proj_coeff_vec_[j][6], proj_coeff_vec_[j][7],  proj_coeff_vec_[j][8],
                                     proj_coeff_vec_[j][9], proj_coeff_vec_[j][10], proj_coeff_vec_[j][11]};
                    }
                    ...
                    else if (cam_ids_.size() == 1)
                    {
                        ...
                        ci_msg->P
                            = {intrinsic_coeff_vec_[j][0], intrinsic_coeff_vec_[j][1], intrinsic_coeff_vec_[j][2], 0,
    ...
    // setting PUBLISH_CAM_INFO_ to true so export to ros method can publish it_.advertiseCamera msg with zero intrisics
    ...
    for (int i = numCameras_ - 1; i >= 0; i--)
    {
        ...
        try
        {
            ...
            if (!soft)
            {
                ...
                {    // sets the configuration for external trigger: used for all slave cameras
    ...
    for (int i = 0; i < numCameras_; i++)
    {
        ...
        if (mkdir(ss.str().c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) < 0)
        {
            ROS_WARN_STREAM("Failed to create directory "
                            << ss.str() << "! Data will be written into pre existing directory if it exists...");
        }
    }
...
void acquisition::Capture::run_soft_trig()
{
    ...
    if (LIVE_)
        namedWindow("Acquisition", cv::WINDOW_NORMAL | cv::WINDOW_KEEPRATIO);
    ...
    try
    {
        while (ros::ok())
        {
            ...
            if ((key & 255) != 255)
            {
                ...
                else if ((key & 255) == 84 && MANUAL_TRIGGER_)
                {    // t
                    cams[MASTER_CAM_].trigger();
                    get_mat_images();
                }
                else if ((key & 255) == 32 && !SAVE_)
                {    // SPACE
            ...
            ROS_INFO_COND(TIME_BENCHMARK_, "total time (ms): %.1f \tPossible FPS: %.1f\tActual FPS: %.1f",
...
void acquisition::Capture::write_queue_to_disk(queue<ImagePtr> *img_q, int cam_no)
{
    ...
    while (imageCnt < k_numImages)
    {
        ...
        ostringstream filename;
        filename << path_ << cam_names_[cam_no] << "/" << cam_names_[cam_no] << "_" << id << "_" << todays_date_ << "_"
                 << std::setfill('0') << std::setw(6) << imageCnt << "_" << timeStamp << ext_;
    ...
    for (int imageCnt = 0; imageCnt < k_numImages; imageCnt++)
    {
        ...
        for (int i = 0; i < numCameras_; i++)
        {
            try
            {
                ...
                filename << cam_names_[i] << "_" << cam_ids_[i].c_str() << "_" << todays_date_ << "_"
                         << std::setfill('0') << std::setw(6) << imageCnt << "_" << timeStamp << ext_;
