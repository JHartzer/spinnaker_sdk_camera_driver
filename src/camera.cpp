...
Mat acquisition::Camera::convert_to_mat(ImagePtr pImage)
{
    ...
    // image data contains padding. When allocating Mat container size, you need to account for the X,Y image data
    // padding.
    Mat img;
    if (COLOR_)
...
bool acquisition::Camera::verifyBinning(int binningDesired)
{
    ...
    if (binningDesired == actualBinningX)
        return true;
    else
        return false;
}
